...
export class InMemoryBugRepository implements BugRepository {
  ...
  const now = Date.now();
  const bug: BugReport = { id, title: input.title, severity: input.severity as any, description: undefined, status: 'open', linkedTaskIds: input.taskId ? [input.taskId] : [], reproSteps: input.reproSteps, proposedFix: input.proposedFix, createdAt: now, updatedAt: now, reporter: undefined } as BugReport;

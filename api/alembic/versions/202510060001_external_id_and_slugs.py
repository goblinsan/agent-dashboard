...
def upgrade() -> None:
    # Add new columns to tasks if missing
    bind = op.get_bind()
    ...
    # Unique index for (milestone_id, slug) when slug present
...
def downgrade() -> None:
    ...
    # Drop index added here (keep conditional to be safe)
    try:
        op.drop_index("ix_tasks_milestone_slug", table_name="tasks")
    except Exception:
        pass

    # Drop columns we added if they exist
    try:
